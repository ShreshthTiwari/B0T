module.exports = {
  name : 'dcannounce',
  description : 'to make a discord announcement',
  ...
  async run(Discord, client, prefix, message, args, database, personFinder, messageEmojiFinder, react, e){
    ...
    let announcementChannelID = await database.get("discordAnnouncementChannelID");
    if(!announcementChannelID){
      embed.setDescription(`${cross} Discord announcement channel not set.`)
        .setColor(0xff4747);
    ...
    if(!announcementChannel){
      embed.setDescription(`${cross} Discord announcement channel not present.`)
        .setColor(0xff4747);
      await message.channel.send(embed).catch(error => {});
      await message.reactions.removeAll();
      react(message, '❌');
    ...
    let pingRoleID = await database.get("discordAnnouncementPingRoleID");
    if(!pingRoleID){
